...
urlpatterns = [
    path('upload/', UploadPYQView.as_view(), name='pyqs_uploaded'),
    path('myuploads/', UserPYQListView.as_view(), name='user-pyqs'),
    path('myuploads/<int:pk>/', EditDeletePYQView.as_view(), name='edit-delete-pyq'),
    path(
        "university/<int_or_all:university_id>/program/<int_or_all:program_id>/branch/<int_or_all:branch_id>/courses/<int_or_all:course_id>/",
        UniversityPYQListView.as_view(),
        name="university-pyqs",
    ),
    path("<int:pyq_id>/ratings/", PYQRatingsView.as_view(), name="pyq-ratings"),
    path("<int:pyq_id>/rate/", RatePYQView.as_view(), name="rate-pyq"),
